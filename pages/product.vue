    ...
    <div v-if="pending" class="flex justify-center items-center py-12">
      <div class="animate-pulse space-y-8 w-full max-w-4xl">
        ...
        <div class="flex flex-col md:flex-row justify-around">
          <div
            class="w-[400px] h-[400px] md:w-[500px] md:h-[500px] bg-gray-200 rounded"
          ></div>
      ...
      <div class="flex flex-col md:flex-row justify-around">
        ...
        <div class="flex flex-col justify-between p-6">
          ...
          <div class="mt-8 md:mt-12">
            ...
            <div class="grid grid-cols-3 gap-4 md:ml-8">
              <div
                ...
                :class="
                  selectedComplements.box
                    ? 'border-[#db9526] ring-2 ring-[#db9526]'
                    : 'border-transparent'
                "
              ...
              >
                ...
                />
                <div
                  class="flex items-center justify-center mt-2 text-xs font-medium"
                >
                  {{ $t("cart.subproducts.box") }} - €{{
                    COMPLEMENTS.find((c) => c.id === "box").price.toFixed(2)
                  }}
                </div>
              </div>
              <div
              ...
              >
                ...
                <div
                  class="flex items-center justify-center mt-2 text-xs font-medium"
                >
                  {{ $t("cart.subproducts.bottle") }} - €{{
                    COMPLEMENTS.find((c) => c.id === "bottle").price.toFixed(2)
                  }}
                </div>
              </div>
              <div
              ...
              >
                ...
                <div
                  class="flex items-center justify-center mt-2 text-xs font-medium"
                >
                  {{ $t("cart.subproducts.card") }} - €{{
                    COMPLEMENTS.find((c) => c.id === "card").price.toFixed(2)
                  }}
                </div>
              </div>
            </div>
          ...
          >
            ...
            <div
              v-if="selectedComplements.box"
              class="flex justify-between items-center"
            >
              <span>1x {{ $t("cart.subproducts.box") }}</span>
              <span
                >€{{
                  COMPLEMENTS.find((c) => c.id === "box").price.toFixed(2)
                }}</span
            ...
            <div
              v-if="selectedComplements.bottle"
              class="flex justify-between items-center"
            >
              <span>1x {{ $t("cart.subproducts.bottle") }}</span>
              <span
                >€{{
                  COMPLEMENTS.find((c) => c.id === "bottle").price.toFixed(2)
                }}</span
            ...
            <div
              v-if="selectedComplements.card"
              class="flex justify-between items-center"
            >
              <span>1x {{ $t("cart.subproducts.card") }}</span>
              <span
                >€{{
                  COMPLEMENTS.find((c) => c.id === "card").price.toFixed(2)
                }}</span
...
function toggleComplement(id) {
  ...
  if (selectedComplements.value[id]) {
    ...
    cart.addItem({
      ...
      images: complement.image
        ? [complement.image.replace(/^public\//, "")]
        : [],
      quantity: 1,
      isComplement: true,
...
function addToCart() {
  ...
  const productWithFullImageUrl = {
    ...
    images: [
      `${config.public.apiBaseUrl}/files/product/${product.value.images}`,
    ],
  };
